// app/o/[slug]/found/page.tsx — « j'ai trouvé un objet » : page ouverte par le
...
async function getOrg(slug: string) {
  ...
  const { data: org } = await sb
    .from("organizations")
    .select("slug, name, city, state_id, verified, public_listing, finder_held_enabled")
...
export default async function OrgFoundPage({ params }: { params: { slug: string } }) {
  const org = await getOrg(params.slug);
  if (!org) notFound();
  ...
  return (
    <main className="mx-auto max-w-xl px-4 py-6 sm:py-10">
      ...
      <OrgFoundForm orgSlug={org.slug} orgName={org.name} publicListing={!!org.public_listing} allowKeep={org.finder_held_enabled !== false} />
    </main>
  );
}
