...
export default function OrgReviewPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const r = await authFetch("/api/org/matches?status=new");
  ...
  const decide = async (id: number, action: "confirm" | "dismiss") => {
    ...
    setError("");
    try {
      const r = await authFetch("/api/org/matches", {
        method: "PATCH",
      ...
      <div className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-6 px-5 py-4">
          <span className="text-[19px] font-bold tracking-tight">
            ReportLost <span className="font-semibold text-gray-400">for organizations</span>
          </span>
          <nav className="flex gap-5 text-[14.5px] text-gray-600">
            ...
            <Link href="/org/dashboard" className="hover:text-gray-900">Inventory</Link>
            <Link href="/org/items/new" className="hover:text-gray-900">Log an item</Link>
          </nav>
          <span className="text-[14px] text-gray-700">{org?.name}</span>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl px-5 py-8">
        ...
        <p className="mt-1.5 text-[15.5px] text-gray-500">
          {matches.length === 0
            ...
            : "A student reported an item that looks like one you hold."}
        </p>
            ...
            {matches.map((m) => (
              <article key={m.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
                ...
                <div className="grid gap-5 p-5 md:grid-cols-2">
                  {/* Ce que l'étudiant a écrit */}
                  <div className="rounded-xl bg-[#f7f8fa] p-4">
                    <p className="text-[12.5px] font-bold text-gray-500">THE STUDENT&apos;S REPORT</p>
                ...
                <div className="flex flex-wrap items-center gap-3 border-t border-gray-100 px-5 py-4">
                  ...
                  >
                    {busy === m.id ? "Saving…" : "Notify the student"}
                  </button>
                </div>
              </article>
            ))}
