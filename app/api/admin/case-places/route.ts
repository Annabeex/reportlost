...
- Termine par UN brouillon de mail type en anglais réutilisable pour ces établissements. RÈGLES DU BROUILLON : sujet contenant #${item.public_id || ""} ; texte brut SANS aucun symbole markdown (pas de **, pas de #, pas de tirets de ponctuation) ; l'adresse de contact à communiquer est l'adresse relais anonyme item${item.public_id || ""}@reportlost.org pour les établissements PRIVÉS (hôtels, commerces...) ; l'email personnel du client ne peut être mentionné que pour un organisme PUBLIC (police, mairie, animal control) si utile ; JAMAIS de placeholder [Client Name] ou [Client Phone/Email] ; inclure la variante "could you kindly forward this to the appropriate department" quand l'adresse email est générique ; signature exacte :
Anna
ReportLost.org
