  ...
  <div style="padding:20px;color:#111827;line-height:1.55;background:#fff">
    ...
    <p style="margin:0 0 8px;font-size:13px;color:#374151">
      Activation is a one-time payment, never a subscription, processed securely by Stripe (PCI DSS v4.0). Your search starts right away.
            ...
            // ❌ E-MAIL CLIENT SUPPRIMÉ À L'ENREGISTREMENT.
            // Ce point du parcours est la fin de l'étape 2 : la personne n'a
            // pas encore vu l'offre. Lui envoyer « One last step to activate
            // your search » revenait à lui adresser un argumentaire pendant
            // qu'elle remplissait encore le formulaire, puis à doubler avec
            // l'e-mail de publication gratuite ou la confirmation de paiement
            // quelques minutes plus tard.
            // Le drapeau `mail_sent` est conservé : il ne signifie plus
            // « e-mail client envoyé » mais « dossier capturé avec une adresse »,
            // et c'est lui qui garantit UNE seule notification à support@.
            const okUser = true;
            if (okUser) {
              try {
                await supabase.from("lost_items").update({ mail_sent: true }).eq("id", existing.id);
                mail_sent = true;
                console.log("📝 Report captured, mail_sent flag persisted for", existing.id);
              } catch (e) {
                console.warn("Could not persist mail_sent flag for existing row:", e);
              }
            } else {
              console.error("❌ Could not persist capture flag for", existing.id);
            }
          } else {
            console.log("⏭️ Already captured, skipping for", existing.id);
          }
        } catch (err) {
      ...
      Activation is a one-time payment, never a subscription, processed securely by Stripe (PCI DSS v4.0). Your search starts right away.
            ...
            if (okUser) {
                ...
                try {
                  ...
                  console.log("📝 Report captured, mail_sent flag persisted for", clientProvidedId);
                } catch (e) {
                  console.warn("Could not persist mail_sent flag for clientProvidedId:", e);
                }
              } else {
                console.error("❌ Could not persist capture flag for", clientProvidedId);
              }
            } else {
              console.log("⏭️ Already captured, skipping for", clientProvidedId);
            }
          } catch (err) {
    ...
    <p style="margin:0 0 8px;font-size:13px;color:#374151">
      Activation is a one-time payment, never a subscription, processed securely by Stripe (PCI DSS v4.0). Your search starts right away.
            ...
            try {
              ...
              console.log("📝 Report captured, mail_sent flag persisted for", insData.id);
            } catch (e) {
              console.warn("Could not persist mail_sent flag for new insert:", e);
            }
          } else {
            console.error("❌ Could not persist capture flag for new insert", insData.id);
          }
        } else {
          console.log("⏭️ Already captured, skipping for", insData.id);
        }
      } catch (err) {
