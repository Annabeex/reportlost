...
export default function FoundItemsForm({ defaultCity = "" }: { defaultCity?: string }) {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      // uploadImageAndAnalyze will upload to storage and call /api/apivision
      ...
      // keep image URL empty if analysis failed (uploadImageAndAnalyze throws on upload failure)
      setVisionImageUrl("");
    } finally {
      clearInterval(progressInterval);
      setProgress(100);
      // small delay so the progress bar finishes visually
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPhoto(file);
    // reset previous vision url when selecting a new file
    setVisionImageUrl("");
    if (file) {
  ...
  const handleSubmit = async () => {
    // prevent double submit while uploading/analyzing/submitting
    ...
    try {
      ...
      // 1) Reuse URL generated by vision (if any), otherwise upload the image here
      let publicImageUrl = visionImageUrl;
      if (!publicImageUrl && photo) {
        // sanitize filename and upload using client supabase
      ...
      // 2) auto title (use same helper as elsewhere)
      const fakeCityData = {
      ...
      // 3) submit to server-side endpoint that uses admin key
      const payload = {
        ...
        <>
          ...
          {autoGenerated && (
            <p className="text-sm text-gray-600 mt-1">
              ⚠️ Please review and complete if needed. Specify where the item was found (e.g., street name, restaurant,
              mall…)
            </p>
          )}
